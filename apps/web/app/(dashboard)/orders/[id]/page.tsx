...
import { useQuotationRevisions } from "@/lib/sales-ops";
...
export default function QuotationDetailPage() {
  ...
  const { data: revisions } = useQuotationRevisions(params.id);
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            {quotation.opportunity ? (
              <Link href={`/active-deals/${quotation.opportunity.id}`} className="text-brand-600 hover:underline">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card className="col-span-2 overflow-hidden p-0">
          ...
          <table className="w-full text-sm">
            <thead className="border-y border-slate-200 bg-slate-50 text-left text-xs font-medium uppercase text-slate-500 dark:border-slate-700 dark:bg-slate-800/50">
              <tr>
          ...
          <div className="ml-auto max-w-xs space-y-1 p-5 text-sm">
            <div className="flex justify-between text-slate-500">
              <span>Subtotal</span>
              <span>
                {quotation.currency} {Number(quotation.subtotal).toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>Discount</span>
              <span>
                -{quotation.currency} {Number(quotation.discountTotal).toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>Tax</span>
              <span>
                {quotation.currency} {Number(quotation.taxTotal).toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between border-t border-slate-200 pt-1 font-semibold text-primary dark:border-slate-700">
              <span>Grand total</span>
              <span>
                {quotation.currency} {Number(quotation.grandTotal).toLocaleString()}
              </span>
            </div>
          </div>
        </Card>

        {(revisions?.versions?.length ?? 0) > 0 && (
          <Card className="col-span-2 p-5">
            <h2 className="mb-3 text-sm font-semibold text-primary">Revision history</h2>
            <ul className="space-y-2 text-sm">
              {revisions.versions.map((v: any) => (
                <li key={v.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-2 last:border-0 dark:border-slate-700">
                  <div>
                    <Link href={`/orders/${v.id}`} className="font-medium text-brand-700 hover:underline dark:text-brand-400">
                      {v.code} v{v.version}
                    </Link>
                    <span className="ml-2 text-slate-500">{v.status.replaceAll("_", " ")}</span>
                    {v.revisionNote && <p className="text-xs text-slate-500">{v.revisionNote}</p>}
                  </div>
                  <span className="text-slate-600">
                    {v.currency} {Number(v.grandTotal).toLocaleString()}
                  </span>
                </li>
              ))}
            </ul>
