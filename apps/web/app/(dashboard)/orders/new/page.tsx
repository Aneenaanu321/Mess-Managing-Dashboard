...
export default function NewQuotationPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/orders/${quotation.id}`);
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="mb-6 text-xl font-semibold text-primary">{getNewItemLabel("/orders")}</h1>

      <Card className="p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2">
              <Label htmlFor="opportunityId">Deal</Label>
              <Select id="opportunityId" required value={opportunityId} onChange={(e) => setOpportunityId(e.target.value)}>
                <option value="">{opportunitiesLoading ? "Loading deals…" : "Select a deal"}</option>
                {opportunities.map((o) => (
              ...
              {!opportunitiesLoading && opportunities.length === 0 && (
                <p className="mt-1 text-xs text-slate-400">No deals yet — create one first.</p>
              )}
            </div>
            <div>
