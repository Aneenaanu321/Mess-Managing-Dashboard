...
export default function PipelinePage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-display text-2xl font-semibold tracking-tight text-slate-900">Pipeline</h1>
      ...
      {!isLoading && !isError && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {OPPORTUNITY_STAGES.map((stage) => (
            <section
              key={stage}
              className={`rounded-2xl border border-slate-200/80 bg-white p-4 shadow-card border-t-[3px] ${COLUMN_STYLE[stage]}`}
            >
              ...
              <div className="space-y-3">
                ...
                {columns[stage].map((opp) => (
                  <Card key={opp.id} className="overflow-hidden p-0 shadow-sm">
                    <Link href={`/opportunities/${opp.id}`} className="block p-3.5 transition-colors hover:bg-slate-50">
                      <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">{opp.code}</p>
                      <p className="mt-1 text-sm font-semibold leading-snug text-slate-900">{opp.title}</p>
      ...
      {lossModalFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <Card className="w-full max-w-sm p-5">
            <h2 className="mb-3 text-sm font-semibold text-slate-900">Mark as Lost</h2>
            <div className="space-y-3">
              <div>
