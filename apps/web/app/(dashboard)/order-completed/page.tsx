  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-700">{getSectionForPage("/order-completed")}</p>
          <h1 className="font-display mt-1 text-2xl font-semibold tracking-tight text-primary">{getPageLabel("/order-completed")}</h1>
          <p className="mt-1 text-sm text-slate-500">
            {data?.meta?.total ?? 0} {getPageLabel("/order-completed").toLowerCase()} created from verified customer orders
          </p>
        </div>
      </div>
                ...
                {salesOrders.map((so: SalesOrder) => (
                  <tr key={so.id} className="transition-colors hover:bg-brand-50/40">
                    <td className="px-4 py-3.5">
                      <Link href={`/order-completed/${so.id}`} className="font-semibold text-brand-700 hover:text-brand-800 hover:underline">
                        {so.code}
                      </Link>
                    </td>
