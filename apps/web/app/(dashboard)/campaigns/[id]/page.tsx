      ...
      <Card className="p-5">
        <dl className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <dd className="mt-0.5 font-medium text-slate-900">{campaign.startDate ? new Date(campaign.startDate).toLocaleDateString() : "—"}</dd>
          </div>
          <div>
            <dt className="text-slate-500">End date</dt>
            <dd className="mt-0.5 font-medium text-slate-900">{campaign.endDate ? new Date(campaign.endDate).toLocaleDateString() : "—"}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Budget</dt>
            <dd className="mt-0.5 font-medium text-slate-900">{campaign.budget ? Number(campaign.budget).toLocaleString() : "—"}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Leads generated</dt>
            <dd className="mt-0.5 font-medium text-slate-900">{campaign._count?.leads ?? 0}</dd>
          </div>
        </dl>
      </Card>
