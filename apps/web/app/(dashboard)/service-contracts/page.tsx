  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-primary">{getPageLabel("/service-contracts")}</h1>
        ...
        {hasPermission(user, "amc:manage") && (
          <Link href="/service-contracts/new">
            <Button>+ {getNewItemLabel("/service-contracts")}</Button>
          </Link>
        )}
      </div>
              ...
              {contracts.map((c: AmcContract) => (
                <tr key={c.id} className={c.expiringSoon ? "bg-amber-50 hover:bg-amber-100" : "hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50"}>
                  <td className="px-4 py-3">
                    <Link href={`/service-contracts/${c.id}`} className="font-medium text-brand-600 hover:underline">
                      {c.code}
                    </Link>
                  </td>
