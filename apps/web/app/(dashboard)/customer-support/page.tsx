  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-primary">{getPageLabel("/customer-support")}</h1>
              ...
              {tickets.map((t: Ticket) => (
                <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50">
                  <td className="px-4 py-3">
                    <Link href={`/customer-support/${t.id}`} className="font-medium text-brand-600 hover:underline">
                      {t.code}
                    </Link>
                  </td>
