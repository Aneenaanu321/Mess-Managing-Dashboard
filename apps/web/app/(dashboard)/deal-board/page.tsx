...
export default function PipelinePage() {
  ...
  if (!canView) {
    return <p className="text-sm text-slate-500">You don&apos;t have permission to view the {getPageLabel("/deal-board").toLowerCase()}.</p>;
  }

  return (
    <div>
      <div className="mb-6">
        <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-700">{getSectionForPage("/deal-board")}</p>
        <h1 className="font-display mt-1 text-2xl font-semibold tracking-tight text-primary">{getPageLabel("/deal-board")}</h1>
        <p className="mt-1 text-sm text-slate-500">
          {opportunities.length} {getPageLabel("/active-deals").toLowerCase()} by stage — open a card to view details.
        </p>
      </div>
              ...
              <div className="space-y-3">
                ...
                {columns[stage].map((opp) => (
                  <Card key={opp.id} className="overflow-hidden p-0 shadow-sm">
                    <Link href={`/active-deals/${opp.id}`} className="block p-3.5 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50">
