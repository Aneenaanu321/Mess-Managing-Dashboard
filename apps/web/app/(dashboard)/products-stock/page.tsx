  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-primary">{getPageLabel("/products-stock")}</h1>
          <p className="text-sm text-slate-500">
            {data?.meta?.total ?? 0} product{data?.meta?.total === 1 ? "" : "s"}
          </p>
        </div>
        {hasPermission(user, "inventory:adjust") && (
          <Link href="/products-stock/new">
            <Button>+ {getNewItemLabel("/products-stock")}</Button>
          </Link>
        )}
      </div>
              ...
              {products.map((product: Product) => (
                <tr key={product.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50">
                  <td className="px-4 py-3">
                    <Link href={`/products-stock/${product.id}`} className="font-medium text-brand-600 hover:underline">
                      {product.sku}
                    </Link>
                  </td>
