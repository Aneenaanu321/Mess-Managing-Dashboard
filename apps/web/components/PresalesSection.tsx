...
export function PresalesSection({ opportunityId }: { opportunityId: string }) {
  ...
  return (
    <Card className="p-5">
      <h2 className="mb-3 text-sm font-semibold text-slate-900">Pre-Sales</h2>
      ...
      <ul className="space-y-2 text-sm">
        {surveys?.map((s) => (
          <li key={s.id} className="border-b border-slate-100 pb-2 last:border-0">
            <p className="font-medium text-slate-900">{new Date(s.surveyDate).toLocaleDateString()}</p>
      ...
      <ul className="space-y-2 text-sm">
        {demos?.map((d) => (
          <li key={d.id} className="border-b border-slate-100 pb-2 last:border-0">
            <p className="font-medium text-slate-900">{new Date(d.demoDate).toLocaleDateString()}</p>
